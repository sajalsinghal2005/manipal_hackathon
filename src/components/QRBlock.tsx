// Decorative QR-style SVG (deterministic from seed). For demo only.
export function QRBlock({ size = 180, seed = "hostelx" }: { size?: number; seed?: string }) {
  const grid = 21;
  const cells: boolean[] = [];
  let h = 0;
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="rounded-xl bg-white p-2">
      {Array.from({ length: grid }).map((_, r) =>
        Array.from({ length: grid }).map((_, c) => {
          ...
          if (!on) return null;
          return <rect key={`${r}-${c}`} x={c * s} y={r * s} width={s} height={s} fill="#0b1226" rx={s * 0.15} />;
        })
      )}
    </svg>
  );
